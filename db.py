...
def default_user_stats() -> dict[str, Any]:
    return {
        "plays": {"round": 0, "coinflip": 0, "mines": 0, "roulette": 0, "slot": 0, "dice": 0},
        "won": {"round": 0, "coinflip": 0, "mines": 0, "roulette": 0, "slot": 0, "dice": 0},
        "lost": {"round": 0, "coinflip": 0, "mines": 0, "roulette": 0, "slot": 0, "dice": 0},
        "totals": {"won": 0, "lost": 0},
...
async def init_db():
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        CREATE TABLE IF NOT EXISTS daily_quest_state(
            ...
            password_plays INTEGER DEFAULT 0,
            ...
            reward_dice INTEGER DEFAULT 0,
            reward_password INTEGER DEFAULT 0,
        ...
        if dq_cols and "password_plays" not in dq_cols:
            await db.execute(
                "ALTER TABLE daily_quest_state ADD COLUMN password_plays INTEGER DEFAULT 0"
            )
        if dq_cols and "reward_password" not in dq_cols:
            await db.execute(
                "ALTER TABLE daily_quest_state ADD COLUMN reward_password INTEGER DEFAULT 0"
            )
        ...
        if dq_cols and "dice_plays" not in dq_cols:
            await db.execute(
                "ALTER TABLE daily_quest_state ADD COLUMN dice_plays INTEGER DEFAULT 0"
            )
        if dq_cols and "reward_dice" not in dq_cols:
            await db.execute(
                "ALTER TABLE daily_quest_state ADD COLUMN reward_dice INTEGER DEFAULT 0"
            )
...
async def _get_daily_row(user_id: int, chat_id: int, day: str):
    async with aiosqlite.connect(DB_NAME) as db:
        cursor = await db.execute(
            """
            SELECT coinflip_count, rob_success, roulette_wins, mines_plays, slot_plays, dice_plays, password_plays,
                   reward_coinflip, reward_rob, reward_roulette, reward_mines, reward_slot, reward_dice, reward_password
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            ...
            INSERT OR IGNORE INTO daily_quest_state(
                user_id, chat_id, day,
                coinflip_count, rob_success, roulette_wins, mines_plays, slot_plays, dice_plays, password_plays,
                reward_coinflip, reward_rob, reward_roulette, reward_mines, reward_slot, reward_dice, reward_password
...
async def daily_quest_bump_dice_play(user_id: int, chat_id: int, reward_amount: int, target: int) -> int:
    """+1 к счётчику /dice (ставка уже проверена ≥ порога)."""
    day = today_str()
    await _ensure_daily_row(user_id, chat_id, day)
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "UPDATE daily_quest_state SET dice_plays = dice_plays + 1 WHERE user_id = ? AND chat_id = ? AND day = ?",
            (user_id, chat_id, day),
        )
        await db.commit()
    row = await _get_daily_row(user_id, chat_id, day)
    ...
    dice_cnt = row[5]
    r_dice = row[12]
    if r_dice or dice_cnt < target:
        return 0
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "UPDATE daily_quest_state SET reward_dice = 1 WHERE user_id = ? AND chat_id = ? AND day = ?",
            (user_id, chat_id, day),
        )
        await db.commit()
    return reward_amount
...
async def daily_quest_bump_password_play(user_id: int, chat_id: int, reward_amount: int, target: int) -> int:
    """+1 к завершённым играм /password (ставка уже проверена ≥ порога)."""
    day = today_str()
    await _ensure_daily_row(user_id, chat_id, day)
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "UPDATE daily_quest_state SET password_plays = password_plays + 1 WHERE user_id = ? AND chat_id = ? AND day = ?",
            (user_id, chat_id, day),
        )
        await db.commit()
    row = await _get_daily_row(user_id, chat_id, day)
    ...
    pw_cnt = row[6]
    r_pw = row[13]
    if r_pw or pw_cnt < target:
        return 0
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "UPDATE daily_quest_state SET reward_password = 1 WHERE user_id = ? AND chat_id = ? AND day = ?",
            (user_id, chat_id, day),
        )
        await db.commit()
    return reward_amount
...
async def get_daily_quest_snapshot(user_id: int, chat_id: int) -> dict[str, Any]:
    ...
    return {
        ...
        "reward_password": reward_password,
    }
